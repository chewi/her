      ...
      class Association
        ...
        def inverse
          if @opts[:inverse_of]
            ...
            raise Her::Errors::AssociationUnknownError.new("Unknown association name :#{iname}") unless inverse
          else
        ...
        def fetch(opts = {})
          ...
          unless result
            result =
              if @parent.new?
                @opts[:default].try(:dup)
              elsif @params.values.include?([]) and !self.is_a?(HasOneAssociation)
                Her::Collection.new
              else
